...
from .tool_executor import ToolExecutor, ToolResult, create_tool_executor, MUTATION_TOOLS, VERIFY_MAP, _action_signature
...
class Alfred:
    ...
    async def execute(
        ...
        # --- Verify any approved actions the client is resending ---
        # Phase B item 2 (ROADMAP.md): the old contract asked the client to
        # send back a bare signature *string*, which only ever worked if
        # the LLM regenerated byte-identical params on this fresh
        # execute() call -- and it has nothing to regenerate them FROM
        # (the original call is never persisted anywhere across the two
        # separate requests an approval round-trip spans), so this never
        # actually worked for any client. New contract: the client echoes
        # back the *whole* awaiting_approval object(s) it received
        # ({"tool", "params", "signature"}). Recompute each signature from
        # its own params here -- never trust a client-supplied signature
        # blindly, only one this process would itself have produced for
        # that exact call -- and only pass the verified ones through.
        preapproved_calls: List[Dict[str, Any]] = []
        approved_signatures: List[str] = []
        for item in (context or {}).get("approved_actions") or []:
            if not isinstance(item, dict):
                continue
            a_tool, a_params, a_sig = item.get("tool"), item.get("params") or {}, item.get("signature")
            if not a_tool or not a_sig:
                continue
            if _action_signature(a_tool, a_params) != a_sig:
                continue  # tampered, stale, or never-actually-offered -- refuse it
            preapproved_calls.append({"tool": a_tool, "params": a_params})
            approved_signatures.append(a_sig)
        ...
        tool_ctx = {
            ...
            "approved_actions": approved_signatures,
        }
        ...
        # --- Run any verified pre-approved actions before the loop starts ---
        # Executed directly with the exact params that were signed -- the
        # LLM is never asked to reproduce the call, which is the actual
        # fix (see the comment above): a fresh signature it emits this
        # turn doesn't need to match anything, because nothing here depends
        # on it emitting one at all. `approved_signatures` already contains
        # this exact call's real signature, so check_guardrails' own
        # membership check passes it through the ordinary tool_executor
        # path -- same execution, verification, and history-recording code
        # a normal in-loop tool call gets, just triggered before turn 1
        # instead of by an LLM tool-call JSON.
        for call in preapproved_calls:
            p_tool, p_params = call["tool"], call["params"]
            thinking.append(f"[Pre-approved] Running {p_tool}")
            result = await self._tool_executor.execute(p_tool, p_params, tool_ctx)
            output = result.output if result.success else result.error or ""
            tools_called.append(p_tool)
            tool_results.append({
                "tool": p_tool, "output": str(output)[:500],
                "success": result.success, "params": p_params,
            })
            if result.success:
                thinking.append(f"  OK ({len(str(output))} chars)")
            else:
                thinking.append(f"  X {p_tool}: {str(output)[:300]}")
            conv.add_tool_result(p_tool, result.to_dict())
            if result.success and self._tool_executor.is_mutation(p_tool, p_params):
                verify_result = await self._tool_executor.verify_mutation(p_tool, tool_ctx, p_params)
                if verify_result:
                    verify_output = verify_result.output if verify_result.success else verify_result.error
                    conv.add_tool_result(f"{p_tool}_verify", {"verification": str(verify_output)[:500]})
                    thinking.append(f"  Verified: {p_tool}")
